%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Runs n simulations of the stochastic SIS model until the runtime is reached, then average.
%
% Inputs:   
%           1/ Hypergraph: I_d and I_e (incidence matrices for edges and hyperedges)
%           2/ Nodes update: beta_d (infection rate from other nodes), beta_e (infection rate from environment), gamma (recovery rate of nodes)
%           3/ Hyperedges update: sigma (contamination rate from nodes), delta (recovery rate of environment)
...
function [xbarsmooth,ybarsmooth] = sim_SIS_average(Id,Ie,beta_d,beta_e,gamma,sigma,delta,p,dt,T,n)
...
%Average degree, hyperdegree, and hyperedge size for the mean-field approximation
kd=2*Md/N;
ke=sum(Ie,"all")/N;
s=sum(Ie,"all")/Me;
...
for t=1:T
    ...
    re=prob_contamination(x(t,:),Ie,sigma); %compute increase risk probability
    for j=1:Me
...
%Update states of mean-field model
for t=1:T
    X(t+1)=X(t)+dt*((beta_d*kd*X(t)+beta_e*ke*Y(t))*(1-X(t))-gamma*X(t));
    Y(t+1)=Y(t)+dt*(sigma*atan(s*X(t))*(1-Y(t))-delta*Y(t));
end
...
plot(xbarsmooth,'r');hold on;
plot(ybarsmooth,'b');
plot(X,'r--');
plot(Y,'b--');
fontsize(12,"points");
xlabel('time');
ylabel('fraction of total nodes/hyperedges');
legend({'{   }$\bar{X}(t)$','{   }$\bar{Y}(t)$','{   }x(t)','{   }y(t)'},'Interpreter','latex');
hold off;shg;
